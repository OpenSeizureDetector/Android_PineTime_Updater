...
This app to update a PineTime watch to use the OpenSeizureDetector (OSD) firmware is based heavily on
the Nordic Seminconductors [DFU Library Example App](https://github.com/NordicSemiconductor/Android-DFU-Library).

The main modifications are that rather than selecting a file from the phone storage, we download the list of possible
firmwares from the OSD server and download the one selected by the user.   We also pre-configure the settings to work
...
Original Nordic Semiconductors Repository README Text
=====================================================
